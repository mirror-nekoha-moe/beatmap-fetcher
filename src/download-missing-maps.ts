...
async function getMissingBeatmapsets() {
    // Query DB for missing maps (missing_audio = true OR downloaded = false)
    const table = process.env.TABLE_BEATMAPSET || 'beatmapset';
    const client = await pool.connect();
    try {
        const res = await client.query(
            `SELECT id FROM public.${table} WHERE missing_audio = true OR downloaded = false ORDER BY id ASC`
        );
        return res.rows;
    } finally {
        client.release();
