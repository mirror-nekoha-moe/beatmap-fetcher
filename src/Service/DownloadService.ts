...
export class DownloadService {
    ...
    // Used for graveyard Maps
    static async downloadBeatmapsetExternal(beatmapsetId: number): Promise<{ filePath: string; fileSize: number }> {
        const mirrors = [
            `https://catboy.best/d/${beatmapsetId}`,
            `https://api.nerinyan.moe/d/${beatmapsetId}`,
            `https://osu.direct/api/d/${beatmapsetId}`,
        ...
        if (!response || !usedUrl) {
            throw new Error("No successful download source");
        }
        ...
        console.log(
            chalk.cyan(
                `Downloaded from ${chalk.whiteBright(usedUrl)} -> ` +
                `${chalk.whiteBright(filePath)} ` +
                `(${chalk.whiteBright((fileSize / (1024 * 1024)).toFixed(2))} MB)`
            )
        );
